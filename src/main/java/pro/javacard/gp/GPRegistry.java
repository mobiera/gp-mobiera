...
public class GPRegistry implements Iterable<GPRegistryEntry> {
	...
	public void add(GPRegistryEntry entry) {
		...
		if (entry instanceof GPRegistryEntryApp) {
			...
			if (entry.getType() == Kind.Application) {
				if (app.getPrivileges().has(Privilege.SecurityDomain)){
	...
	public void parse(byte[] data, Kind type, GPSpec spec) throws GPDataException {
		if (tags) {
			...
			int p1 = 0;
			if (type.equals(Kind.IssuerSecurityDomain)) p1 |= 0x80;
			if (type.equals(Kind.Application)) p1 |= 0x40;
			if (type.equals(Kind.ExecutableLoadFile)) p1 |= 0x20;
	...
	public String toString() {
		...
		String output = "";
		String tab = "     ";
		for (GPRegistryEntry e : this) {
			AID aid = e.getAID();
			output += (e.getType().toShortString() + ": " + HexUtils.bin2hex(aid.getBytes()) + " (" + e.getLifeCycleString() + ")") + "\n";
			if (e.getType() != Kind.IssuerSecurityDomain) {
				output += (" (" + GPUtils.byteArrayToReadableString(aid.getBytes()) + ")") + "\n";
			} else {
				output += "\n";
			}

			if (e.getDomain() != null) {
				output += (tab + "Parent:  " + e.getDomain()) + "\n";
			}
			if (e.getType() == Kind.ExecutableLoadFile) {
				GPRegistryEntryPkg pkg = (GPRegistryEntryPkg) e;
				if (pkg.getVersion() != null) {
					output += (tab + "Version: " + pkg.getVersionString()) + "\n";
				}
				for (AID a : pkg.getModules()) {
					output += (tab + "Applet:  " +  HexUtils.bin2hex(a.getBytes()));
					output += (" (" + GPUtils.byteArrayToReadableString(a.getBytes()) + ")") + "\n";
				}
			} else {
				GPRegistryEntryApp app = (GPRegistryEntryApp) e;
				if (app.getLoadFile() != null) {
					output += (tab + "From:    " + app.getLoadFile()) + "\n";
				}
				if (!app.getPrivileges().isEmpty()) {
					output += (tab + "Privs:   " + app.getPrivileges()) + "\n";
				}
			}
			output += "\n";
		}
